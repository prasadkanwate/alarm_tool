...
###Creating the frame#######
top = Frame(root, width = 700, height = 20, relief = SUNKEN)
top.pack(side = TOP)

left=Frame(root,width=350,height=250,relief=SUNKEN)
left.pack(side=LEFT)

#####creating label of title######

label_info=Label(top,font=('arial',40,'bold'),text="Alarm Tool",fg = "steel blue", bd = 10, anchor = 'w')
label_info.grid(row=0,column=0)

local_time = time.asctime(time.localtime(time.time()))

label_info = Label(top, font = ('arial',10,'bold'), text = local_time, fg = "steel blue", bd = 10, anchor = 'w')
...
def SubmitButton():
  ...
  if AlarmTime == CurrentTime:
     ...
     os.system("start Wecker-sound.mp3")
